...
void classificacao_interna(FILE *arq, int M) {

  rewind(arq); // posiciona cursor no inicio do arquivo
  int reg = 0;
  int nClientes = tamanho_arquivo_cliente(arq);
  int qtdParticoes = 0;
  int t = tamanho_registro_cliente();
  char nome[12];
  char numero[4];
  char extensao[5];
  ...
  while (reg != nClientes) {
    ...
    TCliente *v[M];
    while (!feof(arq)) {
      fseek(arq, (reg)*tamanho_registro_cliente(), SEEK_SET);
      v[i] = le_cliente(arq);
      // print(*v[i]);
      i++;
      reg++;
    ...
    char *nome = malloc(14 * sizeof(char));
    sprintf(nome, "particao%d", qtdParticoes);
    char *fim = ".dat";
    strcat(nome, fim);
    FILE *p;
    ...
    if ((p = fopen(nome, "wb+")) == NULL) {
      ...
      for (int i = 0; i < M; i++) {
        fseek(p, (i)*tamanho_registro_cliente(), SEEK_SET);
        salva_cliente(v[i], p);
        // imprime_cliente(v[i]);
      }
      fclose(p);
    ...
    printf("Particao criada no diretorio.\n");
...
void intercalacao_clientes(char *nome_arquivo_saida, TNomes *nome_particoes) {
  ...
  if ((out = fopen(nome_arquivo_saida, "a")) == NULL) {
    ...
    // TVet v[num_p]; // salva o arquivo da particao e o primeiro cliente
    TVetClientes **v = (TVetClientes **)malloc(num_p * sizeof(TVetClientes));
    TParticao *aux_p =
        nome_particoes->primeiro->prox; // ponteiro para particoes
    printf("Checkpoint intercalacao 2: inicializando vetor\n\n");
    while (aux_p != NULL) { // verifica o primeiro cliente em todas as particoes
    ...
    // escreve os clientes no arquivo final de saida
    int count_null = 0;
    int aux = 0;
    ...
    while (!fim) {
      int menor = INT_MAX;
      int pos_menor = -1; // Inicialize com um valor inválido

      for (i = 0; i < num_p; i++) {
        if (v[i]->cli->cod < menor) {
          menor = v[i]->cli->cod;
          pos_menor = i;
        }
      }

      if (pos_menor != -1) { // Verifica se algum cliente foi encontrado
        fseek(out, aux * tam_reg, SEEK_SET);
        salva_cliente(v[pos_menor]->cli, out);
        v[pos_menor]->aux_p++;

        fseek(v[pos_menor]->f, v[pos_menor]->aux_p * tam_reg, SEEK_SET);
        v[pos_menor]->cli = le_cliente(v[pos_menor]->f);
        aux++;

        if (v[pos_menor]->cli == NULL) {
          v[pos_menor]->cli = cliente(INT_MAX, "Cliente NULL", "XXX.XXX.XXX-XX",
                                      "XX/XX/XXXX", "xxxx@email.com");
        }
      } else {
        fim = 1;
    ...
    TParticao *aux_p2 = nome_particoes->primeiro->prox;
    for (i = 0; i < num_p; i++) {
      v[i]->f = fopen(aux_p2->nome, "r");
      if (v[i]->f != NULL) {
        rewind(v[i]->f);
        TCliente *c2 = le_cliente(v[i]->f);
        ...
        printf("Erro ao abrir arquivo. Contabilizado como nulo.\n");
        count_null++;
      }
      aux_p2 = aux_p2->prox;
    }

    printf("Checkpoint intercalacao 6: fechando arquivos\n");
    // fecha os arquivos
    ...
    printf("Checkpoint intercalacao 7: checando se ainda existem registros\n");
    if (count_null == num_p) {
      printf("Particoes vazias.\n");
      for (i = 0; i < num_p; i++) {
        remove(v[i]->particao->nome);
      }
    }
