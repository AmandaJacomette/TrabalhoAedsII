...
void intercala_particoes_produtos(TVetProd *particao,TVetProd *particao2, TNomes *nome_particoes, int index) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (p1[i] == NULL) {
          p1[i] = produto(INT_MAX, " ", " "); // se não encontrado, define com INT_MAX o codigo
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (p2[i] == NULL) {
          p2[i] = produto(INT_MAX, " ", " ");// se não encontrado, define com INT_MAX o codigo
  ...
  while (!fim) { // escreve no arquivo de saida
    ...
    if (pos_menor != -1) { // verifica se alguma entidade foi encontrada
      ...
      if (n_aux == 1) {
        ...
        if (particao->en == NULL) {
          particao->en = produto(INT_MAX, " ", " "); // se não encontrado, define com INT_MAX o codigo
        }
