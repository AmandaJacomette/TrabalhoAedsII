...
void merge_encomendas(TVetEncomenda **arr, int l, int m, int r) {
  int i, j, k;
  int n1 = m - l + 1;
  int n2 = r - m;

  // Criando arrays temporários para armazenar as duas metades
  TVetEncomenda **L = (TVetEncomenda **)malloc(n1 * sizeof(TVetEncomenda *));
  TVetEncomenda **R = (TVetEncomenda **)malloc(n2 * sizeof(TVetEncomenda *));
...
void mergeSort_encomendas(TVetEncomenda **arr, int l, int r) {
  // printf("\nEntrou no MergeSort.\n");
  if (l < r) {
    int m = l + (r - l) / 2;

    // Ordenando a primeira e a segunda metade do array
    mergeSort_encomendas(arr, l, m);
    mergeSort_encomendas(arr, m + 1, r);

    // Mesclando as duas metades ordenadas
    merge_encomendas(arr, l, m, r);
  }
}
...
void intercalacao_encomendas(char *nome_arquivo_saida, TNomes *nome_particoes) {
  ...
  if ((out = fopen(nome_arquivo_saida, "a+")) == NULL) { // arquivo final aberto em modo "append"
    ...
    TVetEncomenda **v = (TVetEncomenda **)malloc(num_p * sizeof(TVetEncomenda)); // matriz de ponteiros para os vetores da entidade
    ...
    while (aux_p != NULL) { // verifica a primeira entidade em todas as particoes
      ...
      v[i] = (TVetEncomenda *)malloc(sizeof(TVetEncomenda));
      ...
      if (v[i]->f != NULL) {
        ...
        TEncomenda *c = le_encomenda(v[i]->f);

        printf("Leu entidade\n");
        if (c == NULL) { // indicativo do fim de clientes para leitura
          v[i]-> en = encomenda(INT_MAX, " ", " ", " ", " ", "0");
        } else {
          v[i]-> en = c;
        }
        v[i]->aux_p = 0;
    ...
    int n = num_p;
    mergeSort_encomendas(v, 0, n - 1);

    printf("Checkpoint intercalacao 4: mergesort\n");

    // intercala as entidades
    processa_particoes_encomendas(v, nome_particoes, num_p, nome_arquivo_saida);
...
void processa_particoes_encomendas(TVetEncomenda **v, TNomes *nome_particoes, int num_p, char *out) {
  ...
  for (int i = 0; i < num_p; i += 2) { // inicia intercalando em pares os vetores/particoes
    ...
    intercala_particoes_encomendas(v[i], v[j], nome_particoes, (num_p + index));
    index++;
    j += 2;
  }
  ...
  while (!fim) { // nessa segunda iteração, irá verificar e intercalar os vetores que não tiverem sinalizados de "read" (lidos)
    TVetEncomenda *pt1 = NULL;
    TVetEncomenda *pt2 = NULL;
    int pos1 = -1;
    int pos2 = -1;
    read_count = 0;
    ...
    if (pt1 != NULL && pt2 != NULL && pos2 != -1) {
      ...
      intercala_particoes_encomendas(v[pos1], v[pos2], nome_particoes, (num_p + index));
  ...
  sprintf(particao_final, "particoesEncom/particao%d", last);
  char *fim_ext = ".dat";
...
void intercala_particoes_encomendas(TVetEncomenda *particao,TVetEncomenda *particao2, TNomes *nome_particoes, int index) {
  ...
  sprintf(nome, "particoesEncom/particao%d", index);
  char *fim_ext = ".dat";
  strcat(nome, fim_ext);
  ...
  int aux = 0;
  int tam_reg = tamanho_registro_encomenda();
  ...
  for (i = 0; i < 10; i++) {
    p1[i] = le_encomenda(particao->f);
    if (p1[i] == NULL) {
          p1[i] = encomenda(INT_MAX, " ", " ", " ", " ", "0"); // se não encontrado, define com INT_MAX o codigo
  ...
  for (i = 0; i < 10; i++) {
    p2[i] = le_encomenda(particao2->f);
    if (p2[i] == NULL) {
          p2[i] = encomenda(INT_MAX, " ", " ", " ", " ", "0"); // se não encontrado, define com INT_MAX o codigo
  ...
  while (!fim) { // escreve no arquivo de saida
    ...
    if (pos_menor != -1) { // verifica se alguma entidade foi encontrada
      ...
      if (n_aux == 1) {
        ...
        salva_encomenda(p1[pos_menor], out);
        ...
        particao->en = le_encomenda(particao->f);

        if (particao->en == NULL) {
          particao->en = encomenda(INT_MAX, " ", " ", " ", " ", "0"); // se não encontrado, define com INT_MAX o codigo
        }
        ...
        fseek(out, aux * tam_reg, SEEK_SET);
        salva_encomenda(p2[pos_menor], out);
        particao2->aux_p++;

        fseek(particao2->f, particao2->aux_p * tam_reg, SEEK_SET);
        particao2->en = le_encomenda(particao2->f);

        if (particao2->en == NULL) {
          particao2->en = encomenda(INT_MAX, " ", " ", " ", " ", "0");
        }
